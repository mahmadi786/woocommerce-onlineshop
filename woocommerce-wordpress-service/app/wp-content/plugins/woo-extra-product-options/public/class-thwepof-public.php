	...
	***************************************************/
	...
	private function validate_field($valid, $field, $value){
		...
		if($field->is_required() && empty($value)) {
			...
			THWEPOF_Utils::wcpf_add_error(apply_filters('thwepof_required_field_notice', sprintf(THWEPOF_Utils::t('%s is a required field.'), '<strong>'. wp_kses_post($field_label) .'</strong>')));
			$valid = false;
		}else{
	...
	public function validate_min_max_selection($passed, $field, $value){
		...
		if($total_item_checked > 0){
			$min_check = $field->get_property('minlength');
			$max_check = $field->get_property('maxlength');

			if($min_check && ($total_item_checked < $min_check)){
				THWEPOF_Utils::wcpf_add_error('<strong>'. wp_kses_post($field_label) .':</strong> '. sprintf(THWEPOF_Utils::t('Make at least %d selections'), $min_check));
				$passed = false;
			}else if($max_check && $total_item_checked > $max_check){
				THWEPOF_Utils::wcpf_add_error('<strong>'. wp_kses_post($field_label) .':</strong> '. sprintf(THWEPOF_Utils::t('Number of selections exceeds maximum limit (%d)'), $max_check));
				$passed = false;
			}
		}
	...
	public function validate_input_field_maxlength($passed, $field, $value){
		...
		$minlegth = $field->get_property('minlength');
		$word_count = strlen($value);

		if($maxlength && $word_count > $maxlength){
			THWEPOF_Utils::wcpf_add_error('<strong>'. wp_kses_post($field_label) .':</strong> '. sprintf(THWEPOF_Utils::t('The entered value exceeds the maximum length allowed for the field. The maximum number of characters allowed is %d'), $maxlength));
				$passed = false;
		}else if($minlegth && $word_count < $minlegth){
			THWEPOF_Utils::wcpf_add_error('<strong>'. wp_kses_post($field_label) .':</strong> '. sprintf(THWEPOF_Utils::t('The entered value less than the minimum length required for the field. The minimum number of character required is %d'), $minlegth));
				$passed = false;
		}
