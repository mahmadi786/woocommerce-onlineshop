...
class THWEPOF_Utils_Field {
	...
	private static function prepare_field_props($field, $name, $value){
		...
		if($placeholder){
			$props .= ' placeholder="'.esc_attr($placeholder).'"';
		}
	...
	private static function get_html_number($name, $field, $section, $value){
		...
		$props .= is_numeric($field->get_property('minlength')) ? ' min="'. esc_attr($field->get_property('minlength')).'"' : '';
		$props .= is_numeric($field->get_property('maxlength')) ? ' max="'. esc_attr($field->get_property('maxlength')).'"' : '';
		$props .= is_numeric($field->get_property('step')) ? 'step="'. esc_attr($field->get_property('step')).'"' : '';
	...
	private static function get_html_textarea($name, $field, $section, $value){
		...
		$props .= is_numeric($field->get_property('minlength')) ? ' minlength="'. esc_attr($field->get_property('minlength')).'"' : '';
		$props .= is_numeric($field->get_property('maxlength')) ? ' maxlength="'. esc_attr($field->get_property('maxlength')).'"' : '';
	...
	private static function get_html_select($name, $field, $section, $value){
		...
		$input_html .= $placeholder ? '<option value="">'. esc_attr($placeholder) .'</option>' : '';
		foreach($field->get_property('options') as $option_key => $option_text){
			$selected = ($option_text === $value) ? 'selected' : '';
			$input_html .= '<option value="'.esc_attr($option_text).'" '.$selected.'>'.esc_attr($option_text).'</option>';
		}
		$input_html .= '</select>';
	...
	private static function get_html_heading($name, $field, $section, $value){
		...
		$input_html  = '<'.$elm_type.' '.$props.'>'. wp_kses_post($value) .'</'.$elm_type.'>';
