...
public function column_qna_type( $comment ) {
	...
	if ( 0 == $comment->comment_parent ) {
		echo '<img class="cr-qna-list-q-icon" src="' . plugin_dir_url( dirname( dirname( __FILE__ ) ) ) . 'img/question.svg" alt="Question" title="' . __( 'Question', 'customer-reviews-woocommerce' ) . '" />';
	} else {
		echo '<img class="cr-qna-list-q-icon" src="' . plugin_dir_url( dirname( dirname( __FILE__ ) ) ) . 'img/answer.svg" alt="Answer" title="' . __( 'Answer', 'customer-reviews-woocommerce' ) . '" />';
	}
	echo '</div>';
}
