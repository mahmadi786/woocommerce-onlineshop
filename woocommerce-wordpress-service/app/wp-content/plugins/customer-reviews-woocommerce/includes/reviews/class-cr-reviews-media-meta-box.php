...
if ( ! class_exists( 'CR_Reviews_Media_Meta_Box' ) ) :
	...
	class CR_Reviews_Media_Meta_Box {
		...
		public function render_meta_box( $comment ) {
			...
			if ( 0 < $pics_n || 0 < $pics_v_n || 0 < $pics_local_n ) {
				...
				if ( $pics_v_n > 0 ) {
					for ( $i = 0; $i < $pics_v_n; $i++ ) {
						echo '<div id="iv-comment-video-id-' . ($i + 1) . '" class="cr-comment-video">';
						...
						echo '<img class="cr-comment-video-icon" src="' . plugin_dir_url( dirname( dirname( __FILE__ ) ) ) . 'img/video.svg">';
						echo '</div>';
					}
				}
				...
				if ( current_user_can( 'upload_files' ) ) {
					if( isset( $_POST['comment_id'] ) && 0 < $_POST['comment_id'] ) {
						if( isset( $_POST['attachment_id'] ) && 0 < $_POST['attachment_id'] ) {
							if( delete_comment_meta( $_POST['comment_id'], 'ivole_review_image2', $_POST['attachment_id'] ) ) {
